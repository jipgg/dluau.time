...
public:
    ...
    static T& check(lua_State* L, int idx) {
        if (not is(L, idx)) luaL_typeerrorL(L, idx, type_name());
    ...
    static T& create(lua_State* L, T&& v) {
        T* p = static_cast<T*>(lua_newuserdatadtor(L, sizeof(T), [](void* ud) {static_cast<T*>(ud)->~T();}));
        luaL_getmetatable(L, type_name());
        lua_setmetatable(L, -2);
        new (p) T{std::move(v)};
...
private:
    ...
    static int metamethod_index(lua_State* L) {
        ...
        if (found_it == index_.end()) luaL_errorL(L, "unknown field '%s'", key.c_str());
        return found_it->second(L, self);
    }
    static int metamethod_newindex(lua_State* L) {
        ...
        if (found_it == newindex_.end()) luaL_errorL(L, "unknown field '%s'", key.c_str());
        return found_it->second(L, self);
    }
